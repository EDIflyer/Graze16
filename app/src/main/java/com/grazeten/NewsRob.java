...
public class NewsRob extends Application
{
  ...
  static class NewsRobDefaultExceptionHandler implements Thread.UncaughtExceptionHandler
  {
    ...
    public void uncaughtException(final Thread t, final Throwable e)
    {
      ...
      /*
       * StringBuilder sb = new StringBuilder("-- Networks: ("); try { ConnectivityManager cm = (ConnectivityManager) context
       * .getSystemService(CONNECTIVITY_SERVICE);
       * 
       * for (NetworkInfo ni : cm.getAllNetworkInfo()) { sb.append("\n   " + ni.getTypeName() + ": " + ni.getDetailedState() + " - " + ni.getSubtypeName()); }
       * 
       * 
       * } finally { sb.append(")\n"); message.append(sb); }
       */
      ...
      if (Pattern.compile("process has android.permission.WAKE_LOCK").matcher(messageBody).find()
          || Pattern.compile("process has android.permission.ACCESS_NETWORK_STATE").matcher(messageBody).find()
  ...
  {
    final boolean isNRExceptionHandlerAlreadyInstalled = NewsRobDefaultExceptionHandler.class.getSimpleName().equals(
