...
public class MessageHelper
{
  ...
  public static void showMessage(Activity owningActivity, int titleRessourceId, int messageRessourceId, String messageId)
  {
    ...
    try
    {
      ...
      // should be shown
      if (f.createNewFile())
        new AlertDialog.Builder(owningActivity).setIcon(android.R.drawable.ic_dialog_info).setMessage(messageRessourceId)
            .setTitle(titleRessourceId).setPositiveButton(android.R.string.ok, null).create().show();
    }
    catch (IOException e)
    {
    }
