...
public class ArticleListActivity extends AbstractNewsRobListActivity implements IEntryModelUpdateListener, View.OnClickListener
{
  ...
  private class MyListAdapter extends ResourceCursorAdapter
  {
    ...
    public MyListAdapter()
    {
      ...
      readIndicatorBackground = getEntryManager().isLightColorSchemeSelected() ? R.drawable.read_indicator : R.drawable.read_indicator_dark;
    ...
    void bindView(final View view, final Context context, final Cursor cursor, final EntryManager entryManager)
    {
      ...
      final ReadState readState = ReadState.fromInt(DB.getIntegerFromCursor(cursor, columnIndices.readStateIndex));
      ...
      holder.feedTitleTextView.setCompoundDrawablesWithIntrinsicBounds(
          getResources().getDrawable(uiHelper.getArticleDownloadIndicatorDrawable(downloaded, downloadPref, getResources())), null, null, null);

      holder.articleTitleTextView.setText(articleTitle);

      if (NewsRob.SHOW_CHANGED)
      {
        final boolean isStatePending = DB.getBooleanValueFromCursor(cursor, columnIndices.readStatePendingIndex)
      ...
      if (getEntryManager().isRichArticleListEnabled())
      {
        if (holder.thumbnailImageView != null)
        {
          ...
          {
            if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE && U.getScreenSize(context) == 1)
