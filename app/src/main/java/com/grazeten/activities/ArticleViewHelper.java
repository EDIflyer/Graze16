...
class ArticleViewHelper
{
  ...
  static void createArticleMenu(final Menu menu, final Activity owningActivity, final Entry selectedEntry)
  {
    ...
    // .setIntent(Intent.createChooser(createShareArticleSendIntent(selectedEntry), "Share Article")).setIcon(android.R.drawable.ic_menu_share)
    // .setEnabled(alternateHRefAvailable && (selectedEntry.getContent() != null) && (selectedEntry.getContent().length() > 100));
