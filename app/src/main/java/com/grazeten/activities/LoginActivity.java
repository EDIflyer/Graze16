...
public class LoginActivity extends Activity implements OnClickListener
{
  ...
  private void configureView(String captchaToken, String captchaUrl)
  {
    ...
    String email_password_text = getResources().getText(R.string.enter_email_password).toString();
    email_password_text = email_password_text.replaceAll("\\$SERVICE\\$", SyncInterfaceFactory.getSyncInterface(getApplicationContext())
        .getServiceName());
    email_password_text = email_password_text.replaceAll("\\$SERVICE_URL\\$", SyncInterfaceFactory.getSyncInterface(getApplicationContext())
        .getServiceUrl());

    CharSequence styledText = Html.fromHtml(email_password_text);
    descriptionTextView.setText(styledText);
  }
