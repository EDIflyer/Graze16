...
public class UIHelper
{
  ...
  int getArticleDownloadIndicatorDrawable(int downloaded, int downloadPref, Resources resources)
  {
    ...
    int downloadPrefResolved = (downloadPref == Feed.DOWNLOAD_PREF_DEFAULT ? entryManager.getDefaultDownloadPref() : downloadPref);
  ...
  public static String linkize(String url, String s)
  {
    try
    {
      ...
      if (url.indexOf("twitter.com") >= 0 || url.indexOf("facebook.com") >= 0)
      {
        ...
        s = PATTERN_TWITTER_HASH.matcher(s).replaceAll("<a href=\"http://search.twitter.com/search?q=$1\">#$1</a>");
      }
    }
    catch (Exception e)
    {
  ...
  private static void manageWebView(WebView webView, boolean pause)
  {
    ...
    final boolean enabled = "1".equals(NewsRob.getDebugProperties(webView.getContext()).getProperty("pauseWebViews", "1"));
    PL.log("UIHelper.manageWebView called with action: " + methodName + " on webView=" + webView + " enabled=" + enabled, webView.getContext());
    try
    {
