...
public class DBQuery
{
  ...
  public void setShouldHideReadItems(boolean shouldHideReadItems)
  {
    ...
    if (this.hideReadItems != shouldHideReadItems)
    {
      ...
      SDK9Helper.apply(entryManager.getSharedPreferences().edit().putBoolean(EntryManager.SETTINGS_HIDE_READ_ITEMS, shouldHideReadItems));
      changed = true;
    }
  }
  ...
  {
    /*
     * OutputStream output = new OutputStream() { private StringBuilder buffer = new StringBuilder();
     * 
     * @Override public void write(int b) throws IOException { buffer.append((char) b); }
     * 
  ...
  public static DBQuery restore(EntryManager entryManager, InputStream is) throws IOException
  {
    ...
    try
    {
      ...
      Boolean tmpHideReadArticles = p.containsKey("hideReadArticles") ? Boolean.parseBoolean(p.getProperty("hideReadArticles")) : false;
