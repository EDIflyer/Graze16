...
public class APIManager
{
  ...
  public APIManager(final Context context)
  {
    ...
    this.gson = new GsonBuilder().registerTypeAdapter(Date.class, new DateStringTypeAdapter())
        .registerTypeAdapter(Boolean.class, new BooleanTypeAdapter()).registerTypeAdapter(boolean.class, new BooleanTypeAdapter()).create();
  }
   ...
   *          forces a refresh of unread counts. This has a high latency cost and should not be set if the call is being used to display the UI for the first
   *          time, in which case it is more appropriate to make a separate, additional call to refreshFeedCounts().
   */
